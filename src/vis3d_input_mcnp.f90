...
module vis3d_input_mcnp
    use vis3d_constants, only: VIS3D_TAG, VIS3D_MODE_VOXEL, VIS3D_FMT_VTI
    use vis3d_types, only: vis3d_config_t
    use vis3d_input_common, only: vis3d_read_config
    implicit none
    private
    public :: vis3d_read_config_mcnp

contains
    subroutine vis3d_read_config_mcnp(filename, cfg, ierr)
        ...
        do
            ...
            if (ios /= 0) exit
            if (is_vis3d_comment_mcnp(line, payload)) call append_line(lines, payload)
        end do
        close(unit)
        ...
        if (size(lines) == 0) then
            cfg = vis3d_config_t()
            cfg%enabled = .true.
            cfg%mode = VIS3D_MODE_VOXEL
            cfg%format = VIS3D_FMT_VTI
            cfg%output = default_output_path(filename, 'vti')
        ...
        call vis3d_read_config(lines, 'mcnp', cfg, ierr)
    contains
        ...
        logical function is_vis3d_comment_mcnp(line, payload)
            ...
            if (len_trim(t) < 1) then
                is_vis3d_comment_mcnp = .false.; return
            end if
            if (t(1:1) /= 'c' .and. t(1:1) /= 'C') then
                is_vis3d_comment_mcnp = .false.; return
            end if
            t = adjustl(t(2:))
            ...
            is_vis3d_comment_mcnp = .true.
        end function is_vis3d_comment_mcnp
    ...
    end subroutine vis3d_read_config_mcnp
end module vis3d_input_mcnp
