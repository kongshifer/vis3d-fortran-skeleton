...
contains
    subroutine write_vtp(filename, surf, fields, ierr)
        ...
        call xml_write_line(unit, '<VTKFile type="PolyData" version="0.1" byte_order="LittleEndian">')
        write(unit,'(A,I0,A,I0,A)') '  <PolyData><Piece NumberOfPoints="', surf%n_points, '" NumberOfPolys="', surf%n_tris, '">'
        ...
        call xml_write_line(unit, '    </Points>')
        call xml_write_line(unit, '    <Polys>')
        ...
        call xml_write_line(unit, '      </DataArray>')
        call xml_write_line(unit, '    </Polys>')
        call xml_write_line(unit, '  </Piece></PolyData>')
